...
Table: companies
Columns:
  name            TEXT PRIMARY KEY  -- company name
  sector          TEXT              -- e.g. 'Cloud Infrastructure', 'Aerospace'
  founded_year    INTEGER
  employees       INTEGER
  revenue_musd    REAL              -- annual revenue in millions of USD
  headquarters    TEXT              -- "City, ST"
"""
...
class SQLRetriever:
    ...
    question into a SELECT against the known `companies` schema, validates
    it against a small allow-list of guardrails, executes it, and returns
    ...
    def retrieve(self, query: str) -> RetrievalResult:
        ...
        chunks = [RetrievedChunk(text=str(dict(row)), source="companies.db") for row in rows]
